...
void halbb_dig_set_igi_cr(struct bb_info *bb, const struct agc_gaincode_set set)
{
	halbb_set_igi(bb, set.lna_idx, set.tia_idx, set.rxb_idx, RF_PATH_A);
	halbb_set_igi(bb, set.lna_idx, set.tia_idx, set.rxb_idx, RF_PATH_B);
...
void halbb_gaincode_by_rssi(struct bb_info *bb,
			    struct agc_gaincode_set *set, u8 rssi) {
	struct bb_dig_info *bb_dig = &bb->bb_dig_i;
...
void halbb_dig_gaincode_update(struct bb_info *bb)
{
	struct bb_dig_info *bb_dig = &bb->bb_dig_i;
	struct bb_dig_op_unit *bb_dig_u = bb_dig->p_cur_dig_unit;

	/* To obtain proper lna, tia index based on rssi, noisy_lv */
	if (bb_dig_u->dig_op_para.force_gaincode_idx_en) {
		BB_DIG_DBG(bb, DIG_DBG_LV0, "Force gaincode\n");
		return;
	}
	halbb_gaincode_by_rssi(bb, &bb_dig_u->cur_gaincode, bb_dig_u->igi_fa_rssi);
}

void halbb_dig_write_igi(struct bb_info *bb)
{
	struct bb_dig_info *bb_dig = &bb->bb_dig_i;
	struct bb_dig_op_unit *bb_dig_u = bb_dig->p_cur_dig_unit;

	if (bb_dig_u->dig_op_para.force_gaincode_idx_en)
		halbb_dig_set_igi_cr(bb, bb_dig_u->force_gaincode);
	else
		halbb_dig_set_igi_cr(bb, bb_dig_u->cur_gaincode);
}
...
static const u8 igi_rssi_th_ifem_2g[IGI_RSSI_TH_NUM] = {68, 84, 90, 98, 104};
static const u8 igi_rssi_th_ifem_5g[IGI_RSSI_TH_NUM] = {68, 84, 90, 98, 104};
static const u8 igi_rssi_th_efem_2g[IGI_RSSI_TH_NUM] = {68, 84, 90, 98, 104};
static const u8 igi_rssi_th_efem_5g[IGI_RSSI_TH_NUM] = {68, 84, 90, 98, 104};
/* fa_cnt 2000/4000/5000/8000 */
static const u16 fa_th_no_link_2g[FA_TH_NUM] = {196, 352, 440, 528};
static const u16 fa_th_no_link_5g[FA_TH_NUM] = {196, 352, 440, 528};
static const u16 fa_th_linked_2g[FA_TH_NUM] = {4, 8, 12, 16};
static const u16 fa_th_linked_5g[FA_TH_NUM] = {4, 8, 12, 16};
...
void halbb_dig_para_update(struct bb_info *bb)
{
	...
	bool is_2g = (hw_band->cur_chandef.band == BAND_ON_24G) ? true : false;
	...
	if (is_2g) {
		...
		bb_dig->lna_gain = bb_dig->lna_gain_a;
		bb_dig->tia_gain = bb_dig->tia_gain_a;
	}

	/* IGI and PT_low enable control */
	para_dst = &bb_dig->dig_state_h_i.dig_op_para;
	para_dst->force_gaincode_idx_en = true;
	para_dst->dyn_pd_th_en = true;

	if (bb->ic_type == BB_RTL8852A && is_2g)
		para_dst->force_gaincode_idx_en = false;
	...
	case 54:
		if (is_2g)
			halbb_mem_cpy(bb, &para_dst->igi_rssi_th,
				      igi_rssi_th_efem_2g,
				      sizeof(u8) * IGI_RSSI_TH_NUM);
		else
			halbb_mem_cpy(bb, &para_dst->igi_rssi_th,
				      igi_rssi_th_efem_5g,
				      sizeof(u8) * IGI_RSSI_TH_NUM);
	...
	if (is_2g) {
		if (!bb->bb_link_i.is_linked)
			halbb_mem_cpy(bb, &para_dst->fa_th,
				      fa_th_no_link_2g,
...
void halbb_dig_reset(struct bb_info *bb)
{
	...
	halbb_dig_set_igi_cr(bb, bb_dig->max_gaincode);
...
void halbb_dig_lps(struct bb_info *bb)
{
	...
	/* IGI decision */
	halbb_dig_gaincode_update(bb);

	/* To set lna, tia, rxbb index */
	halbb_dig_write_igi(bb);
...
void halbb_dig(struct bb_info *bb)
{
	...
	if (bb_link->first_connect || bb_link->first_disconnect) {
	...
	/* IGI boundary decision */
	igi_new = halbb_dig_igi_bound_decision(bb);
#endif
...
{
	struct bb_dig_info *bb_dig = &bb->bb_dig_i;
	struct agc_gaincode_set pause_gaincode = bb_dig->max_gaincode;
	u8 phy = bb->bb_phy_idx == HW_PHY_1 ? 1 : 0;
	enum channel_width cbw = bb->hal_com->band[phy].cur_chandef.bw;
	...
	BB_DIG_DBG(bb, DIG_DBG_LV0, "[%s] len=%d, val[0]=0x%x, val[1]=0x%x\n",
		   __func__, val_len, val_buf[0], val_buf[1]);

	/* write igi or keep max gaincode */
	if ((enum dig_pause_case)val_buf[1] == PAUSE_OFDM_CCK)
		halbb_gaincode_by_rssi(bb, &pause_gaincode,
				       RSSI_MAX - (u8)val_buf[0]);
	halbb_dig_set_igi_cr(bb, pause_gaincode);

	/* write pd lower bound anyway */
	halbb_set_pd_lower_bound(bb, (u8)val_buf[0], cbw, bb->bb_phy_idx);
}
		   ...
		   char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], "-h") == 0) {
...
#endif
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			    "{1} {fixed IGI en = %d} {fix_lna = %d} {fix_tia = %d} {fix_rxb = %d}\n",
			    bb_dig_u->dig_op_para.force_gaincode_idx_en,
		...
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			    "{15} {0:L,1:H} {fixed IGI en = %d} {fix_lna = %d} {fix_tia = %d} {fix_rxb = %d}\n",
			    bb_dig_u->dig_op_para.force_gaincode_idx_en,
			    bb_dig_u->force_gaincode.lna_idx,
			    bb_dig_u->force_gaincode.tia_idx,
			    bb_dig_u->force_gaincode.rxb_idx);
	...
	if (var[0] == 0) {
		...
		bb_dig_u->dig_op_para.force_gaincode_idx_en = (u8)var[1];
		if (bb_dig_u->dig_op_para.force_gaincode_idx_en) {
		...
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			 "IGI: fix_en|fix(lna,tia,rxb)=%d|(%d,%d,%d)\n",
			 bb_dig_u->dig_op_para.force_gaincode_idx_en,
			 bb_dig_u->force_gaincode.lna_idx,
			 bb_dig_u->force_gaincode.tia_idx,
			 bb_dig_u->force_gaincode.rxb_idx);
		...
		for (i = RSSI_MIN; i <= RSSI_MAX; i++)
			halbb_gaincode_by_rssi(bb, &set_tmp, i);
	} else if (var[0] == 11) {
		...
		if (var[1] == 0)
			...
			bb_dig_u = &bb_dig->dig_state_h_i;
		bb_dig_u->dig_op_para.force_gaincode_idx_en = (u8)var[2];
		if (bb_dig_u->dig_op_para.force_gaincode_idx_en) {
			bb_dig_u->force_gaincode.lna_idx = (u8)var[3];
			bb_dig_u->force_gaincode.tia_idx = (u8)var[4];
			bb_dig_u->force_gaincode.rxb_idx = (u8)var[5];
		}
		bb_dig_u = &bb_dig->dig_state_l_i;
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			 "[TDMA-L]IGI: fix_en|fix(lna,tia,rxb)=%d|(%d,%d,%d)\n",
			 bb_dig_u->dig_op_para.force_gaincode_idx_en,
			 bb_dig_u->force_gaincode.lna_idx,
			 bb_dig_u->force_gaincode.tia_idx,
			 bb_dig_u->force_gaincode.rxb_idx);
		bb_dig_u = &bb_dig->dig_state_h_i;
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			 "[TDMA-H]IGI: fix_en|fix(lna,tia,rxb)=%d|(%d,%d,%d)\n",
			 bb_dig_u->dig_op_para.force_gaincode_idx_en,
			 bb_dig_u->force_gaincode.lna_idx,
			 bb_dig_u->force_gaincode.tia_idx,
			 bb_dig_u->force_gaincode.rxb_idx);
