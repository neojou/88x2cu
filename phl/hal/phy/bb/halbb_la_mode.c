...
void halbb_la_rpt_buf_access(struct bb_info *bb, u32 addr, u32 buff_idx)
{
	...
	page = addr >> 12;
#if 0
	if (page != la->txff_page) {
		la->txff_page = page;
		halbb_set_reg(bb, 0x0140, MASKLWORD, 0x780 + page);
	}
	data_l = halbb_get_32(bb, (0x8000 + (addr & 0xfff)));
	data_h = halbb_get_32(bb, (0x8000 + (addr & 0xfff) + 4));
...
void halbb_la_rpt_buf_get(struct bb_info *bb, u16 finish_ofst, bool is_round_up)
{
	...
	for (i = 0; i < la->smp_number; i++) {
		halbb_la_rpt_buf_access(bb, addr, i << 1);
		addr += 8;
...
void halbb_la_init(struct bb_info *bb)
{
	...
	/* ADCSMP_BUFF_HALF */
	halbb_set_reg(bb, 0x7cc, BIT(30), 0);
		  ...
		  u32 *_out_len)
{
	...
	if ((_os_strcmp(input[1], "-h") == 0)) {
		...
		rpt_tmp = halbb_la_mac_cfg_buf(bb, la->la_mac_cfg_i.mac_la_buf_sel);

		if (!rpt_tmp)
			halbb_la_stop(bb);

		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			    "config_success=%d, Buff_mode=(%d)\n",
			    rpt_tmp, la->la_mac_cfg_i.mac_la_buf_sel);
