...
static u32 odm_read_4byte(struct rf_info *rf, u32 addr)
{
	return hal_read32(rf->hal_com, addr);
}

static void odm_write_1byte(struct rf_info *rf, u32 addr, u8 data)
{
	hal_write8(rf->hal_com, addr, data);
}

static void odm_write_4byte(struct rf_info *rf, u32 addr, u32 data)
{
...
static void odm_set_bb_reg(struct rf_info *rf, u32 addr, u32 mask, u32 data)
{
	u32 value32;
	u32 shift;

	if (mask != 0xFFFFFFFF) {
		value32 = hal_read32(rf->hal_com, addr);
		shift = phy_calculatebitshift(mask);
		data = (value32 & (~mask)) | ((data << shift) & mask);
	}

	hal_write32(rf->hal_com, addr, data);
}

static u32 odm_get_bb_reg(struct rf_info *rf, u32 addr, u32 mask)
{
	u32 value32;
	u32 shift;
	...
	return (value32 & mask) >> shift;
} 

static u32 odm_get_rf_reg(struct rf_info *rf, u8 path, u32 addr)
{
...
static void config_phydm_direct_write_rf_reg_8822c(struct rf_info *rf, u8 path, u32 addr, u32 mask, u32 data)
{
	...
	odm_set_bb_reg(rf, addr, mask, data);
...
static u32 config_phydm_read_rf_reg_8822c(struct rf_info *rf, u8 path, u32 addr, u32 mask)
{
	...
	return odm_get_bb_reg(rf, addr, mask);
}
...
static void odm_set_rf_reg(struct rf_info *rf, u8 path, u32 addr, u32 mask, u32 data)
{
	...
	odm_set_bb_reg(rf, offset[path], 0xFFFFFFFF, data_and_addr);
}
